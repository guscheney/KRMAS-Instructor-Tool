...
(async () => {
  ...
  // ── trigger guards ────────────────────────────────────────────────────
  ev(`state.user = { id: 'u1', role: 'instructor' }; state._userMeta = { tours_seen: ['core-v3'] };`);
  ev(`tourMaybeStart()`);
  await sleep(30);
  ...
  ck('completion persisted to auth metadata', metaWrites.length === 1 && metaWrites[0].tours_seen.includes('core-v3'));
  ck('local metadata updated too', ev(`state._userMeta.tours_seen.includes('core-v3')`));
