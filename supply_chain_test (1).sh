...
-- faithful baseline RLS for stock + movements (mirrors 08/09) so migration 22's
-- read-policy widening for supply admins is actually exercised
alter table public.inventory_stock     enable row level security;
alter table public.inventory_movements enable row level security;
create policy inventory_stock_read on public.inventory_stock for select to authenticated
  using ( is_superadmin() or is_shop_admin() or can_read_scope(school_id) );
create policy inventory_stock_write on public.inventory_stock for all to authenticated
  using ( can_edit_school_stock(school_id) ) with check ( can_edit_school_stock(school_id) );
create policy inventory_movements_read on public.inventory_movements for select to authenticated
  using ( is_superadmin() or is_shop_admin() or can_read_scope(school_id) );
create policy inventory_movements_write on public.inventory_movements for insert to authenticated
  with check ( can_edit_school_stock(school_id) );
...
echo "── F. supply-loc inventory access (read-side fix) ──"
SLOC2=$(val "$SA" "select supply_loc('00000000-0000-0000-0000-0000000000f1');")
chk "supply admin CAN read supply-loc stock" "$(val "$SUP" "select count(*) from inventory_stock where school_id='$SLOC2';")" "2"
chk "supply admin CAN read supply-loc movements" "$(val "$SUP" "select (count(*) > 0)::text from inventory_movements where school_id='$SLOC2';")" "true"
chk "beecroft admin CANNOT read supply-loc stock (isolation)" "$(val "$AB" "select count(*) from inventory_stock where school_id='$SLOC2';")" "0"
no  "beecroft admin CANNOT write supply-loc stock" "$AB" "insert into inventory_stock(school_id,item_id,size,qty) values('$SLOC2','00000000-0000-0000-0000-000000000011','XL',5);"
ok  "supply admin CAN write supply-loc stock" "$SUP" "insert into inventory_stock(school_id,item_id,size,qty) values('$SLOC2','00000000-0000-0000-0000-000000000011','XS',5) on conflict do nothing;"

echo "── G. ship guard: cannot ship beyond finished-goods on-hand ──"
F1='00000000-0000-0000-0000-0000000000f1'
su postgres -c "$PGBIN/psql -p $PORT -d $DB -q" >/dev/null 2>&1 <<SEEDG
insert into public.inventory_items(id,name,supplier_id,sized) values
  ('00000000-0000-0000-0000-000000000013','Gi Pants','$F1',true);
insert into public.inventory_stock(school_id,item_id,size,qty) values
  (public.supply_loc('$F1'),'00000000-0000-0000-0000-000000000013','M',4);
SEEDG
GD='[{"item_id":"00000000-0000-0000-0000-000000000013","item_name":"Gi Pants","size":"M","qty":10,"for_whom":"squad"}]'
OIDG=$(val "$AE" "select supply_order_save(null,'edgeworth','$F1','g','$GD'::jsonb);")
run "$AE" "select supply_order_submit('$OIDG');" >/dev/null 2>&1
run "$SUP" "select supply_order_confirm('$OIDG',null,null);" >/dev/null 2>&1
no  "ship 10 with only 4 on hand is REJECTED" "$SUP" "select supply_order_ship('$OIDG','T','C',null,null);"
chk "status still 'confirmed' after rejected ship (rolled back)" "$(val "$SA" "select status from supply_orders where id='$OIDG';")" "confirmed"
chk "supply-loc stock untouched (still 4)" "$(val "$SA" "select qty from inventory_stock where school_id='$SLOC2' and item_id='00000000-0000-0000-0000-000000000013' and size='M';")" "4"
chk "no movement posted for rejected ship" "$(val "$SA" "select count(*) from inventory_movements where ref_id='$OIDG';")" "0"
LIDG=$(val "$SA" "select id from supply_order_lines where order_id='$OIDG' and size='M';")
OVG="[{\"line_id\":\"$LIDG\",\"qty\":4}]"
ok  "ship 4 (== on hand) succeeds" "$SUP" "select supply_order_ship('$OIDG','T','C',null,'$OVG'::jsonb);"
chk "supply-loc stock now 0 after shipping 4" "$(val "$SA" "select qty from inventory_stock where school_id='$SLOC2' and item_id='00000000-0000-0000-0000-000000000013' and size='M';")" "0"
chk "status shipped after valid ship" "$(val "$SA" "select status from supply_orders where id='$OIDG';")" "shipped"
